...
export function CheckButton({ isDone, onCheck }: CheckButtonProps) {
  return (
    <label className="relative w-14 flex items-center justify-center">
      ...
      />
      <span
        className="w-8 h-8 max-sm:w-7 max-sm:h-7 border border-gray-400 rounded-full flex items-center justify-center peer-checked:border-green-600 transition-all ease-in-out duration-500 cursor-pointer"
